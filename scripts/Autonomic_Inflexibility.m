...
for i = 1:length(filesAndFolders)
    ...
    if isfield(sesdat, 'zmean')
        ...
        if ~isempty(sesdat.zmean.collated_alc)
            alc_labels = sesdat.zmean.collated_alc_labels; % {rat, sex, hemi, ses}
            alc_cue_data = sesdat.zmean.collated_alc(:, 5); % Col 5 is the 10s cue limit (lims4)
        ...
        if ~isempty(sesdat.zmean.collated_soc)
            soc_labels = sesdat.zmean.collated_soc_labels; % {rat, sex, hemi, ses}
            soc_cue_data = sesdat.zmean.collated_soc(:, 5); % Col 5 is the 10s cue limit (lims4)
...
for r = 1:length(ratNames)
    ...
    if length(alc_trials) > 5 % Ensure enough trials to calculate variance
    ...
    if length(soc_trials) > 5 % Ensure enough trials to calculate variance
