...
for i = 1:length(files)
    ...
    valid_trials = traces(:, 2) <= 2 & ~isnan(traces(:, 5));
    ...
    if isEarlySession
        S1_Choice_Alcohol = [S1_Choice_Alcohol; traces(valid_trials & is_alcohol, 5:end)];
        S1_Choice_Social  = [S1_Choice_Social;  traces(valid_trials & is_social, 5:end)];
    
    elseif isLateSession
        S2_Choice_Alcohol = [S2_Choice_Alcohol; traces(valid_trials & is_alcohol, 5:end)];
        S2_Choice_Social  = [S2_Choice_Social;  traces(valid_trials & is_social, 5:end)];
...
time_vector = linspace(-20, 20, size(S1_Choice_Social, 2));
