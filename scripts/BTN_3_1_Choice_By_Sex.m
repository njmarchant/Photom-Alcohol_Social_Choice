...
for i = 1:length(files)
    ...
    if isMale && isPunSession
        M_Choice_Alcohol = [M_Choice_Alcohol; traces(is_alcohol, 5:end)];
        M_Choice_Social  = [M_Choice_Social;  traces(is_social, 5:end)];
    elseif isFemale && isPunSession
        F_Choice_Alcohol = [F_Choice_Alcohol; traces(is_alcohol, 5:end)];
        F_Choice_Social  = [F_Choice_Social;  traces(is_social, 5:end)];
    end
end
...
time_vector = linspace(-20, 20, size(M_Choice_Social, 2));
