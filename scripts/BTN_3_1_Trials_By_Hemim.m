...
for i = 1:length(files)
    ...
    if isLeft && isLateSession
        L_Choice_Alcohol = [L_Choice_Alcohol; traces(valid_trials & is_alcohol, 5:323)];
        L_Choice_Social  = [L_Choice_Social;  traces(valid_trials & is_social, 5:323)];
        L_Choice_Omit    = [L_Choice_Omit;    traces(valid_trials & is_omit, 5:323)];

    elseif isRight && isLateSession
        R_Choice_Alcohol = [R_Choice_Alcohol; traces(valid_trials & is_alcohol, 5:323)];
        R_Choice_Social  = [R_Choice_Social;  traces(valid_trials & is_social, 5:323)];
        R_Choice_Omit    = [R_Choice_Omit;    traces(valid_trials & is_omit, 5:323)];
    end
end
...
time_vector = linspace(-10, 10, size(L_Choice_Social, 2));
